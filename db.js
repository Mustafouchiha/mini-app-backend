...
async function initTables(p) {
  await p.query(`
    ...
    -- Mavjud jadvalga mahalla ustunini qo'shish (agar yo'q bo'lsa)
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'products' AND column_name = 'mahalla'
      ) THEN
        ALTER TABLE products ADD COLUMN mahalla VARCHAR(255) DEFAULT '';
      END IF;
    END $$;

    -- Mavjud jadvalga status ustunini qo'shish (agar yo'q bo'lsa)
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
    ...
    -- Mavjud jadvalga reject_reason ustunini qo'shish (agar yo'q bo'lsa)
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'products' AND column_name = 'reject_reason'
      ) THEN
